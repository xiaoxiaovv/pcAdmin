<template>
  <div class="app-container">
    ...
    <div class="action-container">
      ...
      <el-button type="primary"
                 size="small"
                 @click="openTopConfigDialog">{{ levelAlias.oemName }}支付参数
      </el-button>
      <el-button type="primary"
                 size="small"
                 @click="openCommissionConfigDialog">佣金提现参数
      </el-button>
      <!--<el-button type="primary"
                 size="small"
      ...
      <el-button type="primary"
                 size="small"
                 @click="openTopCommissionDialog">佣金生成
      </el-button>
    </div>
    ...
    <el-table :data="tableData"
      ...
      <el-table-column width="260"
        ...
        <template slot-scope="scope">
          ...
          <el-button type="text"
                     @click="openConfigDialog(scope.row)">支付配置
          </el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog :title="`佣金提现参数配置`"
      ...
      <el-form :model="commissionParam"
        ...
        <el-form-item label="佣金代付">
          <el-switch v-model="commissionParam.isBehalf"
                     :disabled="payDisable"
                     :active-value="1"
                     :inactive-value="0">
          </el-switch>
          <el-button type="warning" size="mini" :disabled="payDisable" v-if="commissionParam.isBehalf" @click="showBehalf">配 置</el-button>
        </el-form-item>
      </el-form>
      <div slot="footer"
           class="dialog-footer">
        ...
        <el-button type="primary"
                   ...
                   :disabled="payDisable">提 交
        </el-button>
      </div>
    </el-dialog>
    <!-- 代付配置 -->
    <el-dialog :title="`佣金代付参数配置`"
               :visible.sync="showBehalfModel"
               width="700px">
      <el-form :model="behalfParam"
               label-width="auto">
             <el-form-item label="代付平台id">
               <el-input v-model.trim="behalfParam.platFormId"
                         placeholder="代付平台id"></el-input>
             </el-form-item>
             <el-form-item label="代付平台登录账号">
               <el-input v-model.trim="behalfParam.username"
                         placeholder="代付平台登录账号"></el-input>
             </el-form-item>
             <el-form-item label="代付平台登录密码">
               <el-input v-model.trim="behalfParam.password"
                         placeholder="代付平台登录密码"></el-input>
             </el-form-item>
             <el-form-item label="客户端id">
               <el-input v-model.trim="behalfParam.clientId"
                         placeholder="客户端id"></el-input>
             </el-form-item>
             <el-form-item label="客户端秘钥">
               <el-input v-model.trim="behalfParam.clientSecret"
                         placeholder="客户端秘钥"></el-input>
             </el-form-item>
             <el-form-item label="平台方公钥">
               <el-input v-model.trim="behalfParam.publicKey"
                         placeholder="平台方公钥"></el-input>
             </el-form-item>
             <el-form-item label="私钥">
               <el-input v-model.trim="behalfParam.privateKey"
                         placeholder="私钥"></el-input>
             </el-form-item>
             <el-form-item label="平台方服务器url">
               <el-input v-model.trim="behalfParam.serverUrl"
                         placeholder="平台方服务器url"></el-input>
             </el-form-item>
            <el-form-item label="平台标示">
              <el-radio-group v-model="behalfParam.type"
                              class="radio-bottom-params">
                <el-radio :label="1">云获</el-radio>
      ...
      <div slot="footer"
           class="dialog-footer">
        <el-button @click="closeBehalf">取 消</el-button>
        <el-button type="primary"
                   @click="submitBehalf"
                   v-loading="btnLoading">提 交
        </el-button>
      </div>
    </el-dialog>
    <!--服务商支付参数配置-->
      ...
      <el-form :model="payParam"
        ...
        <el-form-item label="支付方式">
          <el-radio-group v-model="payParam.payWay"
            ...
            <el-radio label="6"
                      v-if="payConfig.indexOf('富友') > -1">富友
            </el-radio>
            <el-radio label="7"
            ...
            <el-radio label="11"
                      v-if="payConfig.indexOf('传化') > -1">传化
            </el-radio>
            <el-radio label="12"
                      v-if="payConfig.indexOf('天阙随行付') > -1">天阙随行付
            </el-radio>
            <el-radio label="13"
                      v-if="payConfig.indexOf('易生') > -1">易生
            </el-radio>
            <el-radio label="14"
                      v-if="payConfig.indexOf('新大陆') > -1">新大陆
            </el-radio>
            ...
            <el-radio label="17"
                      v-if="payConfig.indexOf('手机pos') > -1">手机pos
            </el-radio>
            <el-radio label="19"
                      v-if="payConfig.indexOf('开店宝') > -1">开店宝
            </el-radio>
            <el-radio label="20"
                      v-if="payConfig.indexOf('畅捷支付') > -1">畅捷
            </el-radio>
            <el-radio label="21"
                      v-if="payConfig.indexOf('敏付') > -1">敏付
            </el-radio>
            <el-radio label="16"
        ...
        <el-form-item label="产品版本号" style="width: 600px; float: left; ">
          ...
          <div v-show="payParam.payWay==16">
            ...
            <el-button type="primary"
                       ...
                       @click="openlklProdRateDialog">拉卡拉费率版本
            </el-button>
            ...
            <el-dialog :title="`拉卡拉版本费率`"
              ...
              <div v-for="(item,index) in payParam.lklProdRateParamTo" :key="index">
                <div class="lklProdRateParamToList">
                  <el-form-item label="版本编号" style="width: 460px;float: left;">
                    <el-input type="text"
                              v-model.trim="item.verCode"
                              :disabled="payDisable"
                    ></el-input>
                  </el-form-item>
                  <el-form-item label="版本费率" style="width: 300px;float: left;">
                    <el-input type="text"
                  ...
                  <div class="setDom">
                    <div class="lklRateTypeListInfoList"
                         @click="lklRateTypeListShow(index)">&nbsp;
                    </div>
                    <div class="delData-list" @click="lklProdRateDialogDelData(1,index)"
                         :data-index="index">
                    </div>
                  </div>
                </div>
                <div v-for="(s,index) in item.lklRateTypeList" :key="index" style="width: 900px; float: left;" v-show="item.isOpen">
                  ...
                  <el-form-item label="费率类型名称" style="width: 300px;float: left;">
                    <el-input type="text"
                  ...
                  <el-form-item label="拉卡拉费率" style="width:280px;float: left; ">
                    <el-input-number :disabled="payDisable"
                                     :precision="2"
                                     :min="0"
                                     :max="100"
              ...
              <div slot="footer"
                   class="dialog-footer">
                <div>
                  <div class="el-button el-button--warning" @click="lklProdRateDialogDelData(0)">新增</div>

                  <el-button type="warning"
                             @click="lklcanConfig">配 置
                  </el-button>
                  <el-button type="primary"
          ...
          <div v-show="payParam.payWay==9">
            ...
            <el-form-item label="所属代理商编号">
              <el-input type="text"
                        v-model.trim="topParam.ysCoOperatorNo"
                        :disabled="payDisable"
                        placeholder="所属代理商编号"></el-input>
            </el-form-item>
          ...
          <el-form-item label="随行付任务编码">
            <div style="display: flex;">
              ...
              <el-button style="margin-left: 10px;"
                         :disabled="payDisable"
                         type="primary"
                         @click="getSxfSubmchidByTaskCode">获取子商户号
              </el-button>
            </div>
            <div v-if="sxFrepoInfoArr.length > 1">
              <el-radio-group v-model="payParam.sxf.subMchId">
                <el-radio :key="item.childNo"
                          :label="item.childNo"
                          v-for="item in sxFrepoInfoArr">{{ item.childNo }}
                </el-radio>
              </el-radio-group>
            </div>
          </el-form-item>
        ...
        <div v-show="payParam.payWay==11">
          ...
          <el-form-item label="商户绑定APPID">
            <el-button type="primary"
                       size="small"
                       :disabled="payDisable"
                       @click="bindChAppId(merchantId, 11)">绑定APPID
            </el-button>
          </el-form-item>
        </div>
        <!--=天阙随行付配置-->
          ...
          <el-form-item label="天阙随行付子商户号">
            <div style="display: flex;">
              ...
              <el-button style="margin-left: 10px;"
                         :disabled="payDisable"
                         type="primary"
                         @click="getTqSxfSubmchidByTaskCode">获取子商户号
              </el-button>
            </div>
...
import {
  ...
  postBehalfData,
  getBehalfData
} from '@/modules/pay/api/pay_config'
...
export default {
  ...
  methods: {
    showBehalf() {
      this.showBehalfModel = true
      getBehalfData().then(res => {
        if(res.obj) {
          this.behalfParam = res.obj
        }
      })
    },
    closeBehalf() {
      this.showBehalfModel = false
    },
    submitBehalf() {
      console.log(this.behalfParam)
      postBehalfData(this.behalfParam).then(res => {
          if(res.code === 200) {
            this.showBehalfModel = false
            this.$message({
              message: res.msg,
    ...
    lklProdRateDialogDelData (e, t) {

      var d = this.$data.payParam['lklProdRateParamTo']
      var n = []
      ...
      if (e == 0) {
        var newsData = {
          ...
          'verRate': '',
          'isOpen': false,
          'lklRateTypeList': this.lkltypelist,
        }
        n.push(newsData)
      } else {
        if(d.length > 1){
      ...
      this.$data.payParam['lklProdRateParamTo'] = n

    },
    lklRateTypeListShow (e) {
      let d = this.$data.payParam['lklProdRateParamTo']
      d[e]['isOpen'] = !d[e]['isOpen']
      this.$data.payParam['lklProdRateParamTo'] = d
    },
    ...
    findTopPayConfig () {
      findTopPayConfig().then(response => {
        ...
        this.topParam.ysCoOperatorNo = data.ysCoOperatorNo
    ...
    */
    ...
    getlklRateTypeArray () {
      getlklRateTypeArray().then(res => {
        this.lkltypelist= res.obj
      }).catch(() => {
        this.loading = false
      })
    },
...
.lklProdRateParamToList .el-form-item{
...
.lklRateTypeListInfoList {
  width: 30px;
  height: 30px;
  background-image: url("https://i.52112.com/icon/jpg/256/20201203/101519/4117409.jpg");
...
.delData-list {
  ...
  border-right: 15px;
  position: relative;
  background-image: url("https://i.52112.com/icon/jpg/256/20150806/834/1230902.jpg");
...
.setDom div:nth-child(1){
  margin-right: 40px;
}
.v-modal{z-index:2006!important;}
</style>
