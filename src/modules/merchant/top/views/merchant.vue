<template>
  <div class="app-container">
    ...
    <div class="action-container">
      <!--商户名-->
      <el-input v-model="searchForm.name" @keyup.enter.native="search" placeholder="商户名" size="small" class="formItem"></el-input>
      <!--联系人-->
      <el-input v-model="searchForm.contact" @keyup.enter.native="search" placeholder="联系人" size="small" class="formItem"></el-input>
      <!--按钮-->
    ...
    <el-table :data="tableData" height="72vh" style="width: 100%">
      ...
      <el-table-column prop="phone" label="商户账号">
      </el-table-column>
      <el-table-column prop="contact" label="联系人">
      </el-table-column>
      <el-table-column prop="phone" width="140" label="联系电话">
      </el-table-column>
      ...
      <el-table-column prop="companyName" label="归属上级">
      </el-table-column>
      <el-table-column prop="managerName" label="归属业务员">
      </el-table-column>
      <el-table-column prop="status" width="60" label="签约状态">
        <template slot-scope="scope">
      ...
      <el-table-column width="300" label="操作">
        <template slot-scope="scope">
          ...
          <template v-if="userType == '1'">
            ...
            <el-button type="text" @click="openWechatConfig(scope.row)">小程序授权</el-button>
            <el-button type="text" @click="deployWechat(scope.row)">小程序提交审核</el-button>
            <el-button type="text" @click="experienceWechat(scope.row)">体验小程序</el-button>
            <el-button type="text" @click="lookWechatStatus(scope.row)">小程序审核状态</el-button>
            <el-button type="text" @click="releaseWechat(scope.row)">小程序正式发布</el-button>
            <el-button type="text" @click="editStatus(scope.row)" v-if="scope.row.signStatus===3">{{scope.row.status===1?'禁用':'启用'}}</el-button>
            <!--<el-button type="text" @click="openAppletConfigDialog(scope.row, true)">微信推荐关注配置</el-button>-->
            <el-button type="text" @click="aliLiftNumConfig(scope.row)">支付宝生活号配置</el-button>
            <!--<el-button type="text"
    ...
    <!--翻页-->
    <pagination :total-elements="totalElements" :change-callback="getMerchantList" ref="page"></pagination>

    <el-dialog title="小程序参数配置" :visible.sync="appletConfigVisible">
      <el-form :model="appletConfig" v-if="appletConfigVisible">
    ...
    <el-dialog title="小程序参数配置" :visible.sync="wechatConfig.show" width="90%">
      <iframe :src="wechatConfig.url" style="width: 100%; height:60vh;" v-if="wechatConfig.url"></iframe>
    </el-dialog>
    <el-dialog title="体验小程序" :visible.sync="experienceInfo.show" v-if="experienceInfo.show">
      <div class="vm-align-center">
        <div class="vm-inline-block vm-height-200px vm-width-200px">
          <img :src="experienceInfo.url" alt="体验小程序" width="100%" height="100%" />
        </div>
      </div>
      <div slot="footer" class="dialog-footer">
        <el-button type="primary" @click="experienceInfo.show = false">关 闭</el-button>
      </div>
    </el-dialog>
    <!--新增模态框-->
    <el-dialog title="新增商户" :close-on-click-modal="false" width="450px" :visible.sync="newMerchant" :before-close="newMerchantClose">

      <el-form :model="newMerchantForm" :rules="Rules" ref="newMerchantForm" :label-width="formLabelWidth" size="mini"
        v-if="newMerchant">
        ...
        <el-form-item label="密码：" show-message>
          <el-input v-model="newMerchantForm.password" type="password" class="formItem" placeholder="可设置初始密码或使用默认密码"></el-input>
        </el-form-item>
        ...
        <el-form-item label="省市：" prop="provInfo">
          <el-cascader v-model="newMerchantForm.provInfo" style="width: 240px" filterable clearable :props="propsInfo"
        ...
        <el-form-item label="经营类别：" prop="businessLevOne">
          <el-cascader v-model="categoryNewArr" class="formItem" clearable :props="propsCategory" :options="typeJson"></el-cascader>
          <el-input v-model="newMerchantForm.businessLevOne" style="display: none;"></el-input>
        </el-form-item>
        <el-form-item label="定位开关">
          <el-switch v-model="newMerchantForm.isOpen" :disabled="true" :active-value="1" :inactive-value="-1">
          </el-switch>
        </el-form-item>
        <el-form-item label="定位地址：" v-show="newMerchantForm.isOpen===1">
          <el-input v-model="locationAddress" placement="请输入商铺地址" class="formItem" id="locationInputId"></el-input>
    ...
    <!--修改模态框-->
    <el-dialog title="修改商户" width="500px" :visible.sync="editMerchant" :before-close="editMerchantClose">

      <el-form :model="editMerchantForm" :rules="Rules" ref="editMerchantForm" :label-width="formLabelWidth" size="mini">
        ...
        <el-form-item label="省市：" prop="provInfo">
          <el-cascader v-model="editMerchantForm.provInfo" ref="locationName" style="width: 240px" filterable clearable :props="propsInfo"
            @change="provInfoChange" :options="provList"></el-cascader>
        </el-form-item>
        <el-form-item label="详细地址：" show-message prop="address">
        ...
        <el-form-item label="经营类别：" prop="businessLevOne">
          <el-cascader v-model="categoryArr" class="formItem" clearable :props="propsCategory" :options="typeJson"></el-cascader>
          <el-input v-model="editMerchantForm.businessLevOne" style="display: none;"></el-input>
        </el-form-item>
        <el-form-item label="定位开关">
          <el-switch v-model="editMerchantForm.isOpen" :disabled="locationDisabledFlag" :active-value="1"
            :inactive-value="-1">
          </el-switch>
        </el-form-item>
        <el-form-item label="定位地址：" v-show="editMerchantForm.isOpen===1">
          <el-input v-model="locationAddress" class="formItem" id="locationInputId"></el-input>
                   <!-- <el-button type="primary" size="small" @click="geoCode">定位</el-button> -->
                   <!-- <el-button type="primary" size="small" @click="geolocationFn">自动定位</el-button> -->
          <!-- <div id="locationContainer" style="width: 100%;height: 200px;margin-top: 10px;margin-bottom: 10px;"></div> -->
        </el-form-item>
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="changeMerchant()" :disabled="btnLoading">确定</el-button>
      </span>
    </el-dialog>
    <!--关注配置-->
    <el-dialog class="vm-dialog vm-dialog-body-top-10px" title="微信推荐关注配置" width="495px" v-if="fallowObj.open"
      :visible.sync="fallowObj.open">
      <FollowConfig :propsInfo="fallowObj"></FollowConfig>
    </el-dialog>
    <!--支付宝-->
    <el-dialog class="vm-dialog vm-dialog-body-top-10px" title="支付宝生活号配置" width="1000px" v-if="dialogAliListNumConfig.open"
      :visible.sync="dialogAliListNumConfig.open">
      <AliLiftConfig :propsInfo="dialogAliListNumConfig" v-if="dialogAliListNumConfig.open"></AliLiftConfig>
    </el-dialog>
  </div>
</template>
...
<script>
  ...
  import {
    turnToWechatConfig,
    auditstatuseWechatApi,
    experienceWechatApi,
    releaseWechatApi
  } from '../api/merchantWechat'
    ...
    methods: {
      ...
      AMapLoader() {
        AMapLoader.load({
          ...
          "plugins": ['AMap.Geocoder', 'AMap.Geolocation', 'AMap.CitySearch','AMap.PlaceSearch', 'AMap.Autocomplete'], // 需要使用的的插件列表，如比例尺'AMap.Scale'等
          ...
          // this.map = new AMap.Map('locationContainer', {
          //   zoom: 13,
          ...
          AMap.plugin(['AMap.Autocomplete','AMap.PlaceSearch'],function(){
            ...
            AMap.event.addListener(autocomplete, 'select', function(e){
              ...
              console.log(e)
              _this.locationAddress = e.poi.district + e.poi.name
              _this.editMerchantForm.longitude = e.poi.location.lng
              _this.editMerchantForm.latitude = e.poi.location.lat
              _this.newMerchantForm.longitude = e.poi.location.lng
              _this.newMerchantForm.latitude = e.poi.location.lat
            })
           })
      ...
      provInfoChange(e) {
         const proName = this.$refs['locationName'].getCheckedNodes()[0].parent.label
         const cityName = this.$refs['locationName'].getCheckedNodes()[0].label
         console.log('aaaaaaaaaaaaa=========', proName,cityName)
         this.locationAddress = proName + cityName
      },
      //坐标转为中文地址
      geolocationFn(){
        var that = this;
        var lnglat = [this.editMerchantForm.longitude, this.editMerchantForm.latitude]
        this.geocoder.getAddress(lnglat, function(status, result) {
          if (status === 'complete' && result.info === 'OK') {
              // result为对应的地理位置详细信息
              console.log(result)
              that.locationAddress = result.regeocode.formattedAddress
          }
        })
      },
      showCityInfo() {
      ...
      rooterDeatil(data) {
        ...
        let id = data.id
        this.$router.push({
          path: '/merchant/top/views/merchantDetail',
          query: {
            id: id
          }
        })
      ...
      sureAdd() {
        ...
        if(!this.locationAddress || !this.newMerchantForm.longitude || !this.newMerchantForm.latitude) {
        ...
        oldForm.locationAddress = this.locationAddress
      ...
      openEditMerchant: function(row) {
        ...
        var newsData = {
          ...
          city:row.city,
          provInfo:[row.province, row.city],
          address:row.address,
          businessLevOne: row.businessLevOne,
          businessLevTwo:row.businessLevTwo,
          businessLevThree:row.businessLevThree,
        ...
        this.locationAddress = row.locationAddress
      ...
      sureModify() {
        if (this.editMerchantForm.isOpen == 1) {
          if(!this.locationAddress || !this.editMerchantForm.longitude || !this.editMerchantForm.latitude) {
            this.$message.warning('请输入定位地址')
            return
          }
        }
