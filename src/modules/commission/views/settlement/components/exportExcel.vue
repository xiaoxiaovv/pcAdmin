<template>
  <div>
    <el-form  ref="validateForm" label-width="100px" :model="params" :rules="rules" @submit.native.prevent>
      ...
      <div style="margin-left: 10px;color: darkorange;text-align: center;">导出结算报表导出的仅为已结算的佣金，请选择导出时间</div>
      <el-form-item>
...
export default {
  ...
  methods: {
    ...
    confirmSubmit () {
      commissionApi.commissionListExportExcel(this.params).then(res => {
        downloadFile(res, '佣金结算报表.xls')
        this.$message.success('恭喜你，佣金结算报表下载成功')
      })
    }
  }
}
