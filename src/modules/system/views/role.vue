<template>
  <div class="app-container" v-loading="loading">
    ...
    <el-table
      ...
      <el-table-column label="公司类型">
        <template slot-scope="scope">
          <template v-if="scope.row.type===1">{{levelAlias.oemName}}</template>
          <template v-else-if="scope.row.type===2">{{levelAlias.firstName}}</template>
          <template v-else-if="scope.row.type===3">{{levelAlias.secondName}}</template>
          <template v-else-if="scope.row.type===4">{{levelAlias.thirdName}}</template>
        </template>
      </el-table-column>
      <!--
      <el-table-column label="角色类型">
        <template slot-scope="scope">
          <template v-if="scope.row.kind===1">业务员</template>
          <template v-else-if="scope.row.kind===2">客服</template>
        </template>
      </el-table-column>
      -->
      <el-table-column label="员工等级">
        <template slot-scope="scope">
          <template v-if="scope.row.level===1">高级</template>
          <template v-else-if="scope.row.level===2">普通</template>
        </template>
      </el-table-column>
      <el-table-column
        prop="createTime"
      ...
      <el-table-column
        ...
        <template slot-scope="scope">
          ...
          <el-button
            size="mini"
            type="primary"
            v-if="!(!scope.row.type || scope.row.type === userType)"
            @click="openNextLevelDialog(scope.row)">下放权限
          </el-button>
          <!-- <el-button
            size="mini"
            type="danger"
    ...
    <el-dialog
      ...
      class="accessDialog">
      <div class="block cirpt-block-height">
    ...
    <el-dialog title="下放权限"
               :visible.sync="nextLeftData.show"
               v-if="nextLeftData.show"
               width="500px">
      <NextLeve :propsInfo="nextLeftData"></NextLeve>
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  created () {
    ...
    this.sessionStorageVal = sessionStorageVal
  },
  mounted () {
  ...
  methods: {
    ...
    getCheckedNodes () {
      ...
      if (userType === undefined || userType === 'undefined') {
        this.$confirm('权限一旦更改会影响所有同层级用户，如需更改，请先联系售后', '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning'
        }).then(() => {
     ...
     * 下放权限
     */
    openNextLevelDialog (data) {
      let params = {
        show: true,
        data: data
      }
      this.$set(this, 'nextLeftData', params)
    }
  }
}
</script>
