    ...
    <div class="header_bar">
      ...
      <div class="block">
        <el-radio-group v-model="searchForm.type" size="small" @change="changeName($event)">
          <el-radio-button label="1" v-if="userType==1">{{levelAlias.firstName}}</el-radio-button>
          <el-radio-button label="2" v-if="userType==1 || userType==2">{{levelAlias.secondName}}</el-radio-button>
...
export default {
  ...
  data () {
    ...
    return {
      ...
      settleData: [
        {
          name: `${levelAlias.secondName}1`, // {{levelAlias.secondName}}名称
          channel: '支付宝', // 返佣渠道
          effective: '1000.00', // 有效交易基数
          effTotal: '10', // 有效交易笔数
          effSettle: '128.00', // 有效佣金
          proportion: '10', // 有效佣金
          belong: '10.00'
        }
      ], // 表格
  ...
  methods: {
    ...
    changeName (ev) {
      if (ev == '0') {
        this.searchName = this.levelAlias.firstName + '名称'
      } else if (ev == '1') {
        this.searchName = this.levelAlias.secondName + '名称'
      } else if (ev == '2') {
