            ...
            <div>
              ...
              <div class="ad-add-box">
                <el-radio-group v-model="addAdForm.type" style="margin-bottom: 30px;">
                  <el-radio-button label="1" class="ad-type-radio">点击量(CPC)</el-radio-button>
                  <el-radio-button label="2" class="ad-type-radio" :disabled="true">展示数(CPM)</el-radio-button>
                  <el-radio-button label="3" class="ad-type-radio" :disabled="true">跳转量(CPV)</el-radio-button>
                </el-radio-group>
              </div>
              <!--不同广告类型-->
              <template>
                <el-form ref="addAdForm" :model="addAdForm" :rules="adRule" size="medium" label-width="120px">
                  ...
                  <el-form-item label="广告图片：" v-if="addAdForm.type=='1' || addAdForm.type=='2' " prop="imageId">
                    ...
                    <span class="tips" style="position: absolute;top: 0;margin-left: 10px">建议上传640*320px<br/>格式为jpg、png、jpeg的图片</span>
                  </el-form-item>
                  <!--注意有表单判断 v-if-->
                  <el-form-item label="推广网址：" v-if="addAdForm.type=='1' || addAdForm.type=='3' " prop="imageLink">
                    <el-input v-model="addAdForm.imageLink" placeholder="输入广告跳转地址" class="ad-input-width"></el-input>
                    <br/>
                    <span class="tips">注意: 请谨慎录入，上线后点击广告图片，将跳转此网站</span>
                  </el-form-item>
            ...
            <div>
              ...
              <el-form :model="addStrategyForm" :rules="adRule" ref="addStrategyForm" label-width="120px">
                <el-form-item label="策略名称：" prop="strategicName">
                  <el-input v-model="addStrategyForm.strategicName" placeholder="输入策略名称"></el-input>
                </el-form-item>
                    ...
                    <el-form-item>
                      ...
                      <div class="cascade-select">
                        <div class="cascade-select-header">
                          <div class="block">
                            <i class="el-icon-delete" style="cursor: pointer"><span
                              class="hearer-user-classify">&nbsp清空</span></i>
                          </div>
                          <div class="block">
                            <el-tooltip content="选择运营商" effect="light" placement="top">
                              <span class="hearer-user-classify">运营商</span>
                            </el-tooltip>
                          </div>
                        </div>
                        <div class="cascade-select-body">
                          <el-checkbox-group v-model="selectOperators">
                            <!--省份选择-->
                            <el-checkbox :label="item.id" v-for="(item,index) in operatorsList"
                                         :key="item.name"
                                         style="width: 100%" @change="selectOpera(item.id)">
                              {{item.name}}
                              <i class="el-icon-arrow-right" v-if="selectOperators.indexOf(item.id)>=0"
                                 style="margin-left: auto" @click.prevent="previewDistribute(item.id)"></i>
                            </el-checkbox>
                          </el-checkbox-group>
                        </div>
                      </div>
                      <!--渠道商-->
                      <div class="cascade-select" v-if="distributePrev" style="margin-left: 10px">
                        <div class="cascade-select-header">
                          <div class="block">
                            <!--<i class="el-icon-delete" style="cursor: pointer"><span-->
                            <!--class="hearer-user-classify">&nbsp清空</span></i>-->
                          ...
                          <div class="block">
                            <el-tooltip content="选择渠道商" effect="light" placement="top">
                              <span class="hearer-user-classify">渠道商</span>
                            </el-tooltip>
                          </div>
                        </div>
                        <div class="cascade-select-body">
                          <el-checkbox-group v-model="selectDistribute">
                            <!--渠道商选择-->
                            <el-checkbox :label="item.id" v-for="(item,index) in currentDistributeList"
                                         :key="item.name"
                                         style="width: 100%" @change="selectDist(item.id)">
                              {{item.name}}
                            </el-checkbox>
                          </el-checkbox-group>
                        </div>
                ...
                <el-form-item label="地域：">
                  ...
                  <!--选择器-->
                  <div v-if="addStrategyForm.regionType==2"
      ...
      <div>
        ...
        <div>
          ...
          <div class="block ">
            ...
            <el-button size="medium" type="primary" @click="resetDay">重置</el-button>
          </div>
        </div>
...
import Rule from '@/assets/city/adRule'
import {
  getOperatorsList,
  getDistributeList,
  saveAdvertise,
  ...
  startPutin,
  getPutInAnalysis
} from '@/modules/extension/api/appAd'
  ...
  filters: {
    ...
    distributorsFil: function (arr) {
      ...
      let opaArr = []
        ...
        let flag = true
          for (let j in opaArr) {
          if (opaArr[j].id == arr[i].operatorId) {
            flag = false
          }
        }
        if (flag) {
          let obj = {}
            obj = {'n': arr[i].operatorName, 'id': arr[i].operatorId, 'chilren': []}
            opaArr.push(obj)
            key++
        }
      }
      ...
      for (let i in opaArr) {
        for (let j in arr) {
          if (arr[j].operatorId == opaArr[i].id) {
            let obj = {}
              obj.id = arr[j].distributorsId
              obj.name = arr[j].distributorsName
  ...
  methods: {
    ...
    backDataformatForm: function (data) {
      this.loading = true
        ...
        for (let i in arr) {
        let flag = true
          for (let j in this.selectOperators) {
          if (this.selectOperators[j].id == arr[i].operatorId) {
            flag = false
          }
        }
        if (flag) {
          let id = arr[i].operatorId
            this.selectOperators.push(id)
          }
      }
      // 渠道商
      this.selectDistribute = []
        let arrdist = data.distributors
        for (let i in arrdist) {
        let id = arrdist[i].distributorsId
          this.selectDistribute.push(id)
        }
    ...
    addStrategySubmit: function () {
      this.btnLoading = true
        ...
        assemble.appType = 2//1支付后广告  2APP广告
    ...
    getOperatorsList: function () {
      getOperatorsList().then(res => {
        console.log(res)
          let data = res.obj
          this.operatorsList = data.content
      }).catch(e => {
    ...
    // 选中某运营商
    selectOpera: function (id) {
      this.currOperatorsDetail.id = id || ''
        getDistributeList(id).then(res => {
        // console.log(res)
        let data = res.obj
          this.currentDistributeList = data.content//赋值渠道商列表
          this.currOperatorsDetail.option = data.content //用于遍历
          //如果没有选过渠道商，则在选中列表中添加渠道商
          if (this.selectOperators.indexOf(id) >= 0) {
          let tmp = data.content
            for (let i in tmp) {
            this.selectDistribute.push(tmp[i].id)
          }
        } else {
          // 如果取消选择，则删除该运营商下的渠道商
          for (let i = 0; i < this.selectDistribute.length; i++) {
            for (let j = 0; j < data.content.length; j++) {
              if (this.selectDistribute[i] == data.content[j].id) {
                let index = this.selectDistribute.indexOf(data.content[j].id)
                  this.selectDistribute.splice(index, 1)
              }
            }
          }
        }

        this.distributePrev = true
      }).catch(e => {

      })
    },
    // 预览运营商下的渠道商
    previewDistribute: function (id) {
      this.currOperatorsDetail.id = id || ''
        getDistributeList(id).then(res => {
        // console.log(res)
        let data = res.obj
    ...
    selectDist: function (data) {
      let count = 0
        let length = this.currOperatorsDetail.option.length || 0
        for (let i in this.selectDistribute) {
        for (let j in this.currOperatorsDetail.option) {
          if (this.selectDistribute[i] === this.currOperatorsDetail.option[j].id) {
            count++
          }
        }
      }
      ...
      if (count <= 0) {
        for (let i in this.selectOperators) {
          if (this.selectOperators[i] == this.currOperatorsDetail.id) {
            this.selectOperators.splice(i, 1)
          }
        }
      } else if (this.selectOperators.indexOf(this.currOperatorsDetail.id) < 0) {
        this.selectOperators.push(this.currOperatorsDetail.id)
      }
    },
