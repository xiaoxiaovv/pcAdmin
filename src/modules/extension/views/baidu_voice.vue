...
<!--      <el-button type="primary" icon="el-icon-plus" size="small" @click="openAddDialog">添加百度语音</el-button>-->
<!--      <el-button icon="el-icon-delete" size="small" @click="deleteBaiduVoices">批量删除</el-button>-->
    ...
    <pagination
      ...
      :change-callback="getBaiduVoiceList"
      ref="page"></pagination>
    <el-dialog
      title="添加百度语音配置"
    ...
    <el-dialog
      ...
      <el-form>
        <el-form-item label="appId" :label-width="formLabelWidth">
          <el-input v-model="currentRow.appId"></el-input>
        </el-form-item>
        <el-form-item label="apiKey" :label-width="formLabelWidth">
          <el-input v-model="currentRow.apiKey"></el-input>
        </el-form-item>
        <el-form-item label="secretKey" :label-width="formLabelWidth">
          <el-input v-model="currentRow.secretKey"></el-input>
        </el-form-item>
      </el-form>
...
export default {
  ...
  methods: {
    ...
    handleDelete (row) {
      ...
      this.deleteBaiduVoices()
    },

    /*
       * 获取SN列表
       * */
    getBaiduVoiceList (number, pageSize) {
      this.loading = true
      getBaiduVoice(number, pageSize).then(response => {
    ...
    addBaiduVoice () {
      ...
      addBaiduVoice(this.form).then(response => {
        this.$message({
    ...
    deleteBaiduVoices () {
      this.$confirm('是否删除百度语音配置？', '提示', {
        ...
        if (this.multipleSelection.length) {
          ...
          deleteBaiduVoice(ids).then(response => {
            this.$message({
              message: response.msg,
              type: 'success',
              duration: 1000
    ...
    openEditDialog (row) {
      ...
      this.currentRow.appId = row.appId
      this.currentRow.apiKey = row.apiKey
      this.currentRow.secretKey = row.secretKey
    ...
    editSnConfig () {
      this.loading = true
      this.btnLoading = true
      updateBaiduVoice(this.currentRow).then(response => {
        this.$message({
          message: response.msg,
          type: 'success',
          duration: 1000
