      ...
      <div>
        ...
        <el-form :model="form" ref="form" :rules="rules" :label-width="labelWidth" :size="inputSize">
          ...
          <el-form-item label="商户二级经营范围：" prop="classify">
            <el-select v-model="form.classify" placeholder="一级经营范围">
          ...
          <el-form-item label="微信费率：" prop="rate_wx">
            <el-input-number v-model="form.rate_wx" :min="0" :precision="3" :max="1" :step="0.001" laceholder="微信费率"
                             class="standard-form"></el-input-number>
            <span class="tips">单位：千分之一 （1~0.001）</span>
          </el-form-item>
          <el-form-item label="支付宝费率：" prop="rate_alipay">
            <el-input-number v-model="form.rate_alipay" :min="0" :precision="3" :max="1" :step="0.001"
                             laceholder="支付宝费率"
                             class="standard-form"></el-input-number>
            <span class="tips">单位：千分之一 （1~0.001）</span>
          </el-form-item>
  ...
  export default {
    ...
    data() {
      ...
      var rate = (rule, value, callback) => {
        let reg = /^-?\d+(\.\d{1,3})?$/;
        if (value > 1) {
          callback(new Error('费率错误，单位千分之一'))
        } else {
          if (!reg.test(value)) {
            callback(new Error('费率错误，单位千分之一'))
          } else {
