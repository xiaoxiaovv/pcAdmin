<template>
  <div class="app-container" v-loading="loading">
      ...
      <el-table-column
        ...
        <template slot-scope="scope">
          ...
          <template v-if="scope.row.isDirect === 1">
            <el-button type="text" :disabled="(scope.row.status===6 || scope.row.status===3 || scope.row.status===1)?false:true" @click="editMemberOpen(scope.row)">修改</el-button>
            <el-button type="text"
                       @click="delOperator(scope.row, 5)"
                       v-if="scope.row.status===3"
                       :disabled="scope.row.status===3?false:true">禁用</el-button>
            <el-button type="text"
                       @click="delOperator(scope.row, 3)"
                       :disabled="scope.row.status===5?false:true"
                       v-else>启用</el-button>
            <!--
            <el-button type="text" @click="OperatorUpdateAudit(scope.row.id,3)"
                       :disabled="scope.row.status===1||scope.row.status===2?false:true">通过审核
            </el-button>
            -->
            <el-button type="text" @click="OperatorUpdateAudit(scope.row.id,2)"
                       :disabled="(scope.row.status===1 || scope.row.status===6)?false:true">提交审核
            </el-button>
            <el-button
              type="text"
    ...
    <el-dialog
      ...
      <el-form :model="newMemberForm" :rules="Rules" ref="ruleForm" :label-width="newFormLabelWidth"
        ...
        <el-form-item label="抽佣比例:" show-message prop="payProrata">
          ...
          %
          <el-tooltip class="item" effect="dark" content="运营商的分佣比例，比如填写60%，则佣金分成60%给运营商" placement="top-start">
    ...
    <el-button type="primary" @click="operaAdd(2)" v-loading="btnLoading" :disabled="btnLoading">确定并提交审核</el-button>
  </span>
    </el-dialog>
    <!--新增模态框-->
    ...
    <!--编辑模态框-->
    <el-dialog
      :title="`编辑${levelAlias.firstName}信息`"
      width="400px"
...
export default {
  ...
  data () {
    ...
    return {
      ...
      statusOption: [{value: 1, name: '未签约'}, {value: 2, name: '待审核'},
        {value: 3, name: '已签约'}, {value: 4, name: '过期'}, {value: 5, name: '禁用'}], // 状态选择器
      ...
      newMemberForm: {
        ...
        address: '', // 详细地址
        // payProrata: '', // 分佣比例【数值】
        status: '' // {{levelAlias.firstName}}状态
      },
      // 编辑
  ...
  methods: {
     ...
     * */
    delOperator: function (data, type) {
      // type 1--启用，0--禁用
    ...
    operaAdd: function (status) {
      this.$refs.ruleForm.validate((valid) => {
        if (valid) {
          this.sureproviderAdd(status)
        } else {
          return false
        }
      })
    },
    sureproviderAdd (status) {
