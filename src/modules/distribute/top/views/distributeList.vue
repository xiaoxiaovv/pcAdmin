    ...
    <el-table
      ...
      <el-table-column
        ...
        <template slot-scope="scope">
          ...
          -->
          <!-- <el-button type="text" @click="distributeUpdateAudit(scope.row.id,3)"
                     :disabled="scope.row.status===2?false:true">审核
          </el-button>

          <el-button type="text" @click="distributeUpdateAudit(scope.row.id,1)"
                     :disabled="scope.row.status===2?false:true">驳回审核
          </el-button> -->
          <!-- <el-button
            type="text"
...
import SetProfitCompent from '@/modules/operator/views/components/setProfit'
...
export default {
  ...
  data () {
    return {
      ...
      statusOption: [{value: 1, name: '未启用'}, {value: 2, name: '待审核'},
        {value: 3, name: '已签约'}, {value: 4, name: '过期'}] // 状态选择器
    }
  },
  mounted () {
  ...
  methods: {
    ...
    * */
    ...
    // 提交审核或者审核
    distributeUpdateAudit (id, status) {
      this.$confirm('是否确认操作？', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
    ...
    rooterDeatil (data) {
      ...
      // query发送ID到详情
      let id = data.id
      this.$router.push({path: '/distribute/top/views/distributeDetail', query: {id: id}})
    },
